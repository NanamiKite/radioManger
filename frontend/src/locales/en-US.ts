export default {
  common: {
    appName: 'RadioManager',
    login: 'Login',
    logout: 'Logout',
    register: 'Register',
    submit: 'Submit',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    back: 'Back',
    home: 'Home',
    search: 'Search',
    loading: 'Loading...',
    error: 'Error',
    success: 'Success',
    warning: 'Warning',
    info: 'Info',
    confirm: 'Confirm',
    yes: 'Yes',
    no: 'No',
    language: 'Language',
    settings: 'Settings',
    profile: 'Profile',
    password: 'Password',
    email: 'Email',
    username: 'Username',
    fullName: 'Full Name',
    timezone: 'Timezone',
    theme: 'Theme',
    dark: 'Dark',
    light: 'Light',
    auto: 'Auto',
    copy: 'Copy',
    download: 'Download',
    upload: 'Upload',
    refresh: 'Refresh',
    export: 'Export',
    import: 'Import',
    operations: 'Operations',
    confirmDelete: 'Confirm Delete',
    editLog: 'Edit Log',
    deleteLog: 'Delete Log',
    details: 'Details',
    selectFile: 'Please select a file',
    imported: 'Imported',
    records: 'records',
    exportStarted: 'Export started',
    exportFailed: 'Export failed',
    importFailed: 'Import failed',
    fileRequired: 'Please select a file',
    deleteSuccess: 'Deleted',
    updateSuccess: 'Updated',
    createSuccess: 'Created',
    noActiveLocation: 'No active location',
    deleteAnyway: 'Delete anyway',
    activated: 'Activated',
    stationLogs: 'Station has {count} log(s)',
    deleteWithoutExport: 'Delete without exporting them first?',
    deleteStationWarning: 'Delete station "{callsign}"?',
    emptyStations: 'No stations yet. Create one to get started.'
  },
  auth: {
    login: 'Login',
    register: 'Register',
    forgotPassword: 'Forgot Password',
    rememberMe: 'Remember Me',
    passwordConfirm: 'Confirm Password',
    alreadyRegistered: 'Already registered? Please',
    noAccount: 'No account? Please',
    logoutSuccess: 'Logout successful',
    invalidCredentials: 'Invalid username or password',
    userExists: 'Username already exists',
    emailExists: 'Email already registered',
    passwordMismatch: 'Passwords do not match',
    passwordTooShort: 'Password must be at least 8 characters',
    welcomeBack: 'Welcome back, {username}!',
    searchCallsign: 'Callsign Lookup',
    searchCallsignHint: 'Query callsign information via QRZ.com',
    enterCallsign: 'Enter callsign',
    lookup: 'Search',
    clear: 'Clear',
    cached: 'Cached',
    live: 'Live',
    notFoundOnQRZ: 'Callsign not found on QRZ.com',
    opensOnQRZ: 'Open on QRZ.com →',
    offlineHint: 'QRZ credentials not configured. Showing approximate DXCC country.',
    registerSuccess: 'Registration successful',
    enterUsername: 'Please enter username',
    enterPassword: 'Please enter password',
  },
  nav: {
    dashboard: 'Dashboard',
    logs: 'QSO Logs',
    stations: 'Stations',
    dxspots: 'DX Spots',
    analysis: 'Analysis',
    callsigns: 'Callsigns',
    shortcuts: 'Shortcuts',
    recycle: 'Recycle Bin',
    tools: 'Tools',
    map: 'Grid Map',
    settings: 'Settings',
    about: 'About'
  },
  dashboard: {
    title: 'Dashboard',
    totalQSO: 'Total QSO',
    dxcc: 'DXCC Countries',
    totalWaz: 'WAZ Zones',
    qslSent: 'QSL Sent',
    qslReceived: 'QSL Received',
    lotwConfirmed: 'LOTW Confirmed',
    totalDistance: 'Total Distance',
    lastQso: 'Last QSO',
    viewLogs: 'View Logs',
    manageStations: 'Manage Stations',
    queryCallsign: 'Query Callsign',
    viewAnalysis: 'View Analysis',
    viewDxcluster: 'DX Cluster',
    viewShortcuts: 'Shortcuts',
    viewRecycleBin: 'Recycle Bin',
    viewSettings: 'Settings',
    recentActivity: 'Recent Activity',
    utcTime: 'UTC+0',
    localTime: 'Local Time',
    monthlyQSO: 'Monthly QSO',
    yearlyQSO: 'Yearly QSO',
    confirmedQSO: 'Confirmed QSO',
    stationCount: 'Stations',
    recentLogs: 'Recent Logs',
    bandDistribution: 'Band Distribution',
    modeDistribution: 'Mode Distribution',
    dxccProgress: 'DXCC Progress',
    worked: 'Worked',
    confirmed: 'Confirmed',
    clickToDetail: 'Click to view detail',
    contestCalendar: 'Contest Calendar',
    today: 'Today'
  },
  udp: {
    listening: 'Listening',
    stopped: 'Stopped',
    wsjtxPort: 'WSJT-X/JTDX Port',
    n1mmPort: 'N1MM Port',
    defaultPort: 'default',
    toggleFailed: 'UDP toggle failed',
    settingsHint: 'In WSJT-X Settings → Reporting, set UDP Server address to this machine IP with matching port. For N1MM, configure in Config → Config Ports → Broadcast Data.'
  },
  map: {
    title: 'Maidenhead Grid Map',
    displayMode: 'Display Mode',
    gridMode: 'Grid Mode',
    radialMode: 'Radial Mode',
    myStation: 'My Station',
    selectedGrid: 'Selected Grid',
    qsoCount: 'QSO Count',
    confirmed: 'Confirmed',
    viewLogs: 'View Logs',
    loadFailed: 'Failed to load map data',
    legend: 'Legend',
    greyline: 'Greyline',
    noQso: 'No QSO'
  },
  tools: {
    title: 'Tools',
    subtitle: 'Amateur Radio Calculation Tools',
    freqWavelength: 'Freq/Wavelength',
    dipoleLength: 'Dipole Antenna',
    verticalLength: 'Vertical Antenna',
    lcFilter: 'LC Filter',
    general: 'General',
    frequencyToWavelength: 'Frequency → Wavelength',
    wavelengthToFrequency: 'Wavelength → Frequency',
    frequency: 'Frequency',
    wavelength: 'Wavelength',
    halfWave: 'Half-wave',
    quarterWave: 'Quarter-wave',
    velocityFactor: 'Velocity Factor (VF)',
    wireType: 'Wire Type',
    vertical: 'Element',
    verticalElement: 'Vertical Element Length',
    radial: 'Radial',
    radialType: 'Radial Length Type',
    radialQuarter: 'λ/4 (Ground-mounted, ~50Ω)',
    radialHalf: 'λ/2 (Elevated, ~36Ω)',
    radialHint: 'Use λ/4 for ground-mounted, λ/2 for elevated',
    radialLength: 'Single Radial Length',
    radialsTotal: 'Total Radial Wire',
    numRadials: 'Number of Radials',
    bandTable: 'Band Reference',
    sameAsVertical: 'Same as element',
    vfHint: 'Bare copper ≈ 0.95, insulated wire ≈ 0.66-0.90',
    vfHintVertical: 'Aluminum tubing ≈ 0.95, copper wire ≈ 0.95',
    totalLength: 'Total Length',
    eachLeg: 'Each Leg',
    quarterWaveLength: 'Quarter-wave Length',
    length: 'Length',
    feedpoint: 'Feed Point',
    insulator: 'Insulator',
    radials: 'Radials',
    hamBands: 'Amateur Band Reference',
    dipoleBandTable: 'Dipole Band Quick Reference (VF=0.95)',
    fromImpedance: 'From Impedance',
    fromSWR: 'From SWR',
    feedpointImpedance: 'Antenna Z (ZL)',
    feedlineImpedance: 'Feedline Z (Z0)',
    reflectedPower: 'Reflected Power',
    returnLoss: 'Return Loss',
    mismatchLoss: 'Mismatch Loss',
    referenceTable: 'Reference',
    status: 'Status',
    filterType: 'Filter Type',
    filterDesign: 'Design Method',
    lowpass: 'Low-Pass',
    highpass: 'High-Pass',
    cutoffFreq: 'Cutoff Frequency',
    impedance: 'Impedance',
    numComponents: 'Components',
    ripple: 'Passband Ripple',
    componentValues: 'Component Values',
    dbConverter: 'dB Power Converter',
    powerIn: 'Power In',
    powerOut: 'Power Out',
    wireGauge: 'Wire Gauge',
    diameter: 'Diameter',
    resistance: 'Resistance',
    maxAmps: 'Max Amps',
    formula: 'Formula',
    source: 'Source',
    and: 'and',
    or: 'or',
    transmissionLines: 'Transmission Lines'
  },
  logs: {
    title: 'QSO Logs',
    newLog: 'New Log',
    callSign: 'Call Sign',
    qsoDate: 'QSO Date',
    frequency: 'Frequency',
    band: 'Band',
    mode: 'Mode',
    rstSent: 'RST Sent',
    rstReceived: 'RST Received',
    gridSquare: 'Grid Square',
    qslSent: 'QSL Sent',
    qslReceived: 'QSL Received',
    comment: 'Comment',
    noLogs: 'No logs yet',
    addLog: 'Add Log',
    editLog: 'Edit Log',
    deleteLog: 'Delete Log',
    exportLogs: 'Export Logs',
    importLogs: 'Import Logs',
    statistics: 'Statistics',
    logDetail: 'Log Detail',
    freqMhz: 'Freq (MHz)',
    allBands: 'All Bands',
    allModes: 'All Modes',
    allStations: 'All Stations',
    allDxcc: 'All DXCC',
    location: 'Location',
    allLocations: 'All Locations',
    exportStation: 'Export Station Logs',
    exportNoStation: 'Please select a station',
    exportLocation: 'Export Location Logs',
    exportGridFilter: 'Export only logs matching this location grid (my_gridsquare)',
    editLogTitle: 'Edit QSO Log',
    createLogTitle: 'New QSO Log',
    confirmDeleteMsg: 'Are you sure you want to delete this log?',
    stationId: 'Station',
    yes: 'Yes',
    no: 'No',
    requested: 'Requested',
    ignore: 'Ignore',
    timeUtc: 'Time (UTC)',
    utcHint: 'UTC+0',
    logNotFound: 'Log not found',
    mhz: ' MHz',
    pleaseEnterCallsign: 'Please enter callsign',
    pleaseSelectDate: 'Please select date',
    pleaseSelectBand: 'Please select band',
    pleaseSelectMode: 'Please select mode',
    noActiveStation: 'No active station set, showing all logs',
    active: 'Active',
    deCall: 'De Call',
    lotwConfirmed: 'LoTW Confirmed',
    dropFile: 'Drop .adi/.adif file here or',
    clickToBrowse: 'click to browse',
    importResult: 'Imported: {imported} | Duplicates: {duplicates} | QSL Updated: {updated} | Errors: {errors}',
    line: 'Line',
    error: 'Error',
    format: 'Format',
    noActiveStationShort: 'No active station',
    noStationWarning: 'No active station with a configured location. Please create a station and location first.',
  },
  dxcluster: {
    title: 'DX Cluster',
    connect: 'Connect',
    disconnect: 'Disconnect',
    spotsAmount: 'Spots Amount',
    currentNode: 'Current Node',
    connected: 'Connected',
    disconnected: 'Disconnected',
    nodes: 'Choose spot node',
    recentSpots: 'Receiving spots',
    freq: 'Frequency',
    band: 'Band',
    callsign: 'Callsign',
    dxcc: 'DXCC',
    mode: 'Mode',
    spotter: 'Spotter',
    comment: 'Comment',
    time: 'Time',
    noData:"No spots yet",
    noSpots: 'No spots yet',
    selectNode: 'Select Node',
    connecting: 'Connecting',
    connectFailed: 'Connect failed',
  },
  stations: {
    title: 'Stations',
    newStation: 'Add Station',
    newLocation: 'New Location',
    manageStations: 'Manage Stations and Locations',
    callSign: 'Call Sign',
    gridSquare: 'Grid Square',
    radioModel: 'Radio Model',
    antennaModel: 'Antenna Model',
    qth: 'QTH',
    primary: 'Primary',
    secondary: 'Secondary',
    noStations: 'No stations yet,create one to get started.',
    addStation: 'Add Station',
    editStation: 'Edit Station',
    deleteStation: 'Delete Station',
    confirmDeleteMsg: 'Delete station {callsign}?',
    cannotDeleteLast: 'Cannot delete the last station',
    primaryTag: 'Primary',
    secondaryTag: 'Secondary',
    editLocation: 'Edit Location',
    name: 'Name',
    namePlaceholder: 'e.g. Home, Mobile, Field',
    antennaHeight: 'Antenna Height (m)',
    required: 'Required',
    deleteAnyway: 'Delete anyway',
    cancel: 'Cancel',
    cannotDeleteStation: 'Cannot delete this station',
    deleteLocation: 'Delete location "{name}"?',
  },
  analysis: {
    title: 'Analysis',
    statistics: 'Statistics',
    charts: 'Charts',
    bands: 'Band Distribution',
    modes: 'Mode Distribution',
    countries: 'Country Distribution',
    timeRange: 'Time Range',
    refreshStats: 'Refresh Statistics',
    qslStats: 'QSL Statistics',
    activity: 'Activity',
    totalQso: 'Total QSOs',
    dxccEntities: 'DXCC Entities',
    lastQso: 'Last QSO',
    avgDistance: 'Avg Distance',
    km: 'km',
    bandModeMatrix: 'Band × Mode Matrix',
    noData: 'No data',
    filterEntity: 'Filter entity...',
    allBands: 'All Bands',
    all: 'All',
    confirmed: 'Confirmed',
    worked: 'Worked',
    notWorked: 'Not Worked',
    band: 'Band',
    mode: 'Mode',
  },
  settings: {
    title: 'Settings',
    account: 'Account',
    preferences: 'Preferences',
    security: 'Security',
    about: 'About',
    udpSettings: 'UDP Settings',
    language: 'Language',
    timezone: 'Timezone',
    theme: 'Theme',
    notifications: 'Notifications',
    saveSettings: 'Save Settings',
    settingsSaved: 'Settings saved',
    changePassword: 'Change Password',
    currentPassword: 'Current Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm Password',
    passwordChanged: 'Password changed',
    joined: 'Joined',
    role: 'Role',
    appName: 'RadioManager',
    version: 'Version',
    dbMode: 'Database Mode',
    apiServer: 'API Server',
    manageAccount: 'Manage your account and preferences',
    description: 'Amateur Radio Log Management System',
    designedBy: 'Designed By',
    sourceCode: 'Source Code',
    contact: 'Contact',
    copyright: 'Copyright',
    allRightsReserved: 'All rights reserved',
    dbPath: 'Database Path',
    copyPath: 'Copy Path',
    openFolder: 'Open Folder',
    pathCopied: 'Path copied, open it in File Explorer',
    enterPasswordConfirm: 'Enter password to confirm',
    lightMode: 'Light Mode',
    darkMode: 'Dark Mode',
    followSystem: 'Follow System',
  },
  shortcuts: {
    title: 'Shortcuts',
    subtitle: 'Quick links to amateur radio websites',
    noShortcuts: 'No shortcuts configured'
  },
  recycleBin: {
    title: 'Recycle Bin',
    recycleBinTip: 'Deleted logs are kept for 7 days before permanent removal',
    recycleBinEmpty: 'Recycle bin is empty',
    clearAll: 'Clear All',
    date: 'Date',
    reason: 'Reason',
    expires: 'Expires',
    days: '{count} days',
    expired: 'Expired',
    restore: 'Restore',
    loadFailed: 'Failed to load recycle bin',
    restored: 'Restored log: {callSign}',
    restoreFailed: 'Restore failed',
    batchDeleteConfirm: 'Permanently delete {count} items? This cannot be undone.',
    batchDeleted: 'Deleted {count} items',
    deleteFailed: 'Delete failed',
    clearAllConfirm: 'This will permanently delete ALL {count} items in the recycle bin. This cannot be undone!',
    clearAllTitle: 'Clear Recycle Bin',
    finalConfirm: 'Are you absolutely sure? All {count} items will be permanently deleted.',
    finalConfirmTitle: 'Final Confirmation',
    continue: 'Continue',
    yesDeleteAll: 'Yes, Delete All',
    cleared: 'Cleared {count} items from recycle bin',
    clearFailed: 'Clear failed',
  },
  admin: {
    section: 'Admin',
    userManagement: 'User Management',
    userManagementDesc: 'Manage system user accounts',
    searchUser: 'Search username or email',
    active: 'Active',
    disabled: 'Disabled',
    status: 'Status',
    stats: 'Stats',
    disable: 'Disable',
    enable: 'Enable',
    userStats: 'User Stats',
    resetPassword: 'Reset Password',
    systemStatus: 'System Status',
    systemStatusDesc: 'View system health and metrics',
    dbMode: 'Database Mode',
    totalUsers: 'Total Users',
    activeUsers: 'Active Users',
    onlineSessions: 'Online Sessions',
    systemConfig: 'System Config',
    configKey: 'Key',
    configDesc: 'Description',
    configValue: 'Value',
    auditLog: 'Audit Log',
    auditLogDesc: 'View system operation records',
    time: 'Time',
    detail: 'Detail',
    deleteAccount: 'Delete Account',
    deleteAccountDesc: 'Your account will enter a 30-day cooldown period. You can cancel during this period. Data will be permanently deleted after cooldown.',
    deleteAccountCooldown: 'Account deletion requested. You have 30 days to cancel.',
    cancelDelete: 'Cancel Deletion',
    confirmDelete: 'Confirm Deletion',
    role: 'Role',
    roleUser: 'user',
    roleAdmin: 'admin',
    id: 'ID',
    loadFailed: 'Failed to load users',
    toggleSuccess: 'User {username} has been {action}',
    toggleFailed: 'Failed to toggle user',
    actionDisable: 'disabled',
    actionEnable: 'enabled',
    deleteConfirm: 'Delete user {username}?',
    deleteSuccess: 'User {username} deleted',
    deleteFailed: 'Failed to delete user',
    loadStatsFailed: 'Failed to load stats',
    configUpdated: 'Config updated',
    configUpdateFailed: 'Failed to update config',
    username: 'Username',
    action: 'Action',
    user: 'User',
    target: 'Target',
    ipAddress: 'IP Address',
    loadAuditFailed: 'Failed to load audit logs',
    type: 'Type',
  },
  callsign: {
    fullName: 'Full Name',
    country: 'Country',
    gridSquare: 'Grid Square',
    address: 'Address',
    class: 'License Class',
    previousCalls: 'Previous Callsigns',
    licenseDate: 'License Date',
    licenseExp: 'License Expiry',
    cqZone: 'CQ Zone',
    ituZone: 'ITU Zone',
    email: 'Email',
    coordinates: 'Coordinates'
  },
  validation: {
    required: 'This field is required',
    email: 'Please enter a valid email address',
    callSign: 'Please enter a valid call sign',
    gridSquare: 'Please enter a valid grid square',
    passwordTooShort: 'Password must be at least 8 characters',
    passwordMismatch: 'Passwords do not match',
    invalidFrequency: 'Invalid frequency',
    invalidBand: 'Invalid band'
  },
  errors: {
    404: 'Page not found',
    500: 'Server error',
    networkError: 'Network error',
    unauthorized: 'Unauthorized',
    forbidden: 'Forbidden',
    serverError: 'Server error, please try again later'
  },
  search: {
    typeCallsign: 'Callsign',
    typeGrid: 'Grid',
    typeDxcc: 'DXCC',
    placeholderCallsign: 'Search callsign...',
    placeholderGrid: 'Search grid...',
    placeholderDxcc: 'Select DXCC entity...',
  }
}
