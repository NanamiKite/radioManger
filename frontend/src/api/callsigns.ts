...
export const callsignsApi = {
  lookup(call_sign: string): Promise<CallsignInfo> {
    return api.get(`/callsigns/${encodeURIComponent(call_sign)}`)
  },
  ...
  search(prefix: string, country?: string): Promise<{ results: CallsignInfo[] }> {
    ...
    return api.get(`/callsigns/search/${encodeURIComponent(prefix)}`, { params })
  },

  clearCache(call_sign: string): Promise<void> {
    return api.delete(`/callsigns/cache/${encodeURIComponent(call_sign)}`)
  }
}
